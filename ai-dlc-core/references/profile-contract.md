...
one repo — `apps/utser`, `packages/utse_*`, `src/*-se`. A description that says only "for
...
Then `aidlc init <slug> --profile flutter-utser`, which records the profile in the state
